...
# Try to get weights directory from environment, fallback to relative path
weights_dir = os.environ.get('WEIGHTS_DIR', None)
if weights_dir and os.path.exists(os.path.join(weights_dir, 'ProteinMPNN_v48_noise_0.2.pt')):
    default_ckpt = os.path.join(weights_dir, 'ProteinMPNN_v48_noise_0.2.pt')
else:
    # Fallback: try relative to script location (for Docker)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(script_dir)  # Go up from scripts/ to project root
    default_ckpt = os.path.join(project_dir, 'weights', 'ProteinMPNN_v48_noise_0.2.pt')
    if not os.path.exists(default_ckpt):
        # Last resort: Docker path
        default_ckpt = '/home/weights/ProteinMPNN_v48_noise_0.2.pt'
