...
def _get_usalign_command():
    """
    Get the command to run USalign, using GLIBC wrapper if in HPC environment
    """
    # Check if we're in HPC environment with GLIBC wrapper
    glibc_loader = os.environ.get('GLIBC_LOADER')
    glibc_lib_path = os.environ.get('RFANTIBODY_GLIBC_LIBRARY_PATH')
    
    if glibc_loader and glibc_lib_path and os.path.exists(glibc_loader):
        # Return command with GLIBC wrapper
        return [glibc_loader, '--library-path', glibc_lib_path, usalign]
    else:
        # Direct execution
        return [usalign]
...
def tmalign_to_subset(pose1: "Pose", pose2: "Pose", subset: str) -> None:
    ...
    usalign_cmd = _get_usalign_command()
    result = subprocess.run(usalign_cmd + [output1, output2, '-o', tag], 
                           stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    retlines = result.stdout.decode('utf-8').split('\n')
    
    # Extract TM-scores, handle USalign failure gracefully
    tm_scores = [float(l.split(' ')[1].strip()) for l in retlines if l.startswith('TM-score=')]
    
    if not tm_scores:
        # USalign failed (likely due to library version incompatibility)
        print(f'WARNING: USalign failed for {subset} alignment - library version incompatible')
        print(f'Skipping TM-score calculation, continuing RF2 prediction...')
        ...
        for f in glob.glob(f'{tag}*'):
            try:
                ...
                pass
        return None  # Return None to skip alignment, RF2 continues
