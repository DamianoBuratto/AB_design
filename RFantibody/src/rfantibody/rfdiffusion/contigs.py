...
class ContigMap():
    ...
    '''
    def __init__(self, parsed_pdb, contigs=None, inpaint_seq=None, inpaint_str=None, length=None, ref_idx=None, hal_idx=None, idx_rf=None, inpaint_seq_tensor=None, inpaint_str_tensor=None, topo=False, provide_seq=None):
        #sanity checks
        ...
        if provide_seq is not None:
            for i in provide_seq[0].split(","):
                if "-" in i:
                    self.inpaint_seq[int(i.split("-")[0]):int(i.split("-")[1])+1] = True
                else:
