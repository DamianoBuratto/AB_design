...
Stage 4: SCORING (MMPBSA)
  Batch Run → Result Collection
  Output: Binding energy ranking

Stage 5: AFFINITY (FEP)         
...
| Script | Function |
|--------|----------|
| `setup_mmpbsa_env.sh` | Create the `gmxMMPBSA` conda environment |
| `inputs/mmpbsa_LinearPB.in` | Main gmx_MMPBSA input file (Linear PB) |
| `scripts/auto_submit_mmpbsa.py` | Main submit tool: auto-manages SLURM queue |
| `scripts/collect_mmpbsa_results.py` | Collect MMPBSA results into summary tables |
